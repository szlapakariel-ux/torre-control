...
**Sobre las carpetas `remotas/` y `reportes-remotos/`** (introducidas con la implementación de Torre Central multi-repo, ORD-2026-04-25-12): el detector solo mira `historial/`, no esas carpetas. La presencia de órdenes encoladas en `remotas/` o reportes recibidos en `reportes-remotos/` **no afecta** el veredicto de ciclo cerrado. Un futuro ciclo puede agregar un check secundario que alerte sobre órdenes remotas sin transportar hace mucho; por ahora queda fuera de scope.
